<template>
    <h1 class="titulo">CmpGrupoUsuario</h1>
        ...
        <form>
            ...
            <Button label="Enviar" icon="pi pi-check" class="p-button-success" @click="submitNew" />
            <Button label="Cancelar" icon="pi pi-times" class="p-button-secondary" @click="cancelNew" />
            </div>
        </form>
        </Dialog>
        <!-- Diálogo de confirmación para eliminar un grupo -->
        <Dialog v-model:visible="displayConfirmDialog" header="Confirmar eliminación" :modal="true">
        <p>¿Estás seguro de que deseas eliminar el grupo "{{ selectedGrupo.nombre_grupo }}"?</p>
        <div class="p-dialog-footer">
            <Button label="Cancelar" icon="pi pi-times" class="p-button-secondary" @click="cancelDelete" />
            <Button label="Aceptar" icon="pi pi-check" class="p-button-danger" @click="deleteGrupo" />
        </div>
        </Dialog>
        <!-- Diálogo para editar un grupo -->
        <Dialog v-model:visible="displayEditDialog" header="Editar Grupo" :modal="true">
        <form>
            <div class="p-field">
            ...
            <br />
            <div class="p-dialog-footer">
            <Button label="Guardar" icon="pi pi-check" class="p-button-success" @click="updateGrupo" />
            <Button label="Cancelar" icon="pi pi-times" class="p-button-secondary" @click="cancelEdit" />
            </div>
        </form>
        </Dialog>
</template>
...
export default {
    data() {
        return {
            ...
            newGrupo: { // Objeto para almacenar los datos del nuevo grupo
            nombre_grupo: '',
            },
            ...
            editData: { // Datos del grupo seleccionado para editar
            id_grupo: null,
            nombre_grupo: '',
            },
    ...
    deleteGrupo() {
        if (this.selectedGrupo) {
            ...
            axios.delete('http://127.0.0.1:5000/grupoUsuario', { data: { id_grupo_usuario: idGrupo } })
                .then(response => {
                // Eliminar el grupo de la lista actual de grupos
                const index = this.grupos.findIndex(grupo => grupo.id_grupo === idGrupo);
                if (index !== -1) {
    ...
    submitNew() {
        axios.put('http://127.0.0.1:5000/grupoUsuario', this.newGrupo)
        .then(response => {
    ...
    updateGrupo() {
        if (this.editData.id_grupo) {
            ...
            axios.patch('http://127.0.0.1:5000/grupoUsuario', data)
            .then(response => {
                // Actualizar el grupo en la lista actual de grupos
                const index = this.grupos.findIndex(grupo => grupo.id_grupo === this.editData.id_grupo);
                if (index !== -1) {
